...
Complete reference for layout and structural components in Universal UI. These components handle content organization, scrolling, rendering, and portal management.
...
Web-only component that customizes browser scrollbar appearance to match your theme. Renders nothing on native platforms.

### When to Use

- Customizing scrollbar colors on web
- Matching scrollbar to application theme
- Providing consistent UI across browsers

### Props

```typescript
interface ScrollBarProps {
  thumbColor?: string;    // Scrollbar thumb color (default: theme.colors.border)
  trackColor?: string;    // Scrollbar track color (default: "transparent")
}
```

### How It Works

On web, injects CSS styles into the document head to customize:
- Webkit scrollbar (Chrome, Safari, Edge)
- Standard scrollbar (Firefox)

On native (iOS/Android), renders nothing and has no effect.

### Basic Example

```typescript
import { ScrollBar } from "@korsolutions/ui";
...
```typescript
import { ScrollBar, useTheme } from "@korsolutions/ui";
...
```typescript
// app/_layout.tsx
import { ScrollBar, UniversalUIProvider } from "@korsolutions/ui";
...
- **Web**: Injects CSS styles into document head
- **iOS/Android**: No-op, renders nothing
- **Cleanup**: Automatically removes styles on unmount
- **Multiple Instances**: Last mounted instance wins (avoid multiple instances)

### Best Practices

1. Mount once at root level
2. Use theme colors for consistency
3. Update colors when theme changes
4. Keep track color transparent for modern look
5. Don't use on native platforms (automatic)
